...
test('店铺详情弹窗使用更宽的容器宽度', () => {
  ...
  assert.match(className, /max-w-\[1400px\]/);
  assert.doesNotMatch(className, /max-w-4xl/);
});

test('店铺详情弹窗左右栏使用更舒展的比例', () => {
  const classNames = getStoreDetailModalPaneClassNames();

  assert.match(classNames.historyPane, /w-\[56%\]/);
  assert.match(classNames.formPane, /w-\[44%\]/);
